    ...
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur border-t border-slate-200 px-2 py-1 shadow-lg">
      <div className="flex items-center justify-around">
        {primaryTabs.map((tab) => {
          ...
          let colorClass = isActive ? 'text-saffron-600 font-bold' : 'text-slate-500 hover:text-slate-800';
          if (tab.isAlert) {
            colorClass = isActive ? 'text-rose-600 font-extrabold' : 'text-rose-500 font-semibold';
          }
